...
int main(int argc, char* argv[])
{
	...
	printf(" xmax-xmin: %f, ymax-ymin: %f,  yres: %d\n", ymax-ymin, xmax-xmin,  yres);
	...
	double dx = (xmax - xmin) / xres;				// dx: 0.0000001465
	double dy = (ymax - ymin) / yres;				// dy: 0.0000001466
	...
	for (j = 0; j < yres; j++)		// ROWS
	{
		y = ymax - j * dy;
		for(i = 0; i < xres; i++)		// COLS
		{
			double u = 0.0;
			double v= 0.0;
			double u2 = u * u;
